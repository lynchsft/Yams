...
class TagCodingTests: XCTestCase {
    ...
    /// Test the encoding of a a type that does not conform to YamlTagProviding but none the less declares a coding member with the same name
    ...
    /// Nothing interesting happens when a type does not conform to YamlTagProviding none the less declares a coding member with the same name but that value is nil
    ...
    /// If the decoded type does not conform to YamlTagProviding it can still have a coding key called `yamlTag`
    /// If Yams tries to decode such a type AND the document has a nil value for `yamlTag` AND the parent context is a mapping AND that mapping has an actual tag (in the document)
...
class TagWithAnchorCodingTests: XCTestCase {
    ...
    /// If types conform to YamlTagProviding and are NOT Hashable-Equal then HashableAliasingStrategy does not alias them
    ...
    /// If types conform to YamlTagProviding can declare to have the same tag and still be NOT Hashable-Equal then HashableAliasingStrategy does not alias them
    /// even though their members may still be Hashable-Equal and therefor maybe aliased.
    ...
    /// If different types conform to YamlTagProviding they can declare to have the same tag and further, have exactly the same encoded representation.
    /// In thisi case StrictEncodableAliasingStrategy will still alias them even though they are encoded and decoded from different types.
...
fileprivate struct SimpleWithTag: SimpleProtocol, YamlTagProviding {
    let nested: NestedStruct
    let intValue: Int
    var yamlTag: Tag? = "simple"
}

fileprivate struct SimpleWithTag2: SimpleProtocol, YamlTagProviding {
    let nested: NestedStruct
    let intValue: Int
    var yamlTag: Tag? = "simple"
}

fileprivate struct SimpleWithoutTag: SimpleProtocol {
    let nested: NestedStruct
    let intValue: Int
}

fileprivate struct SimpleWithStringTypeTagName: SimpleProtocol {
    let nested: NestedStruct
    let intValue: Int
    var yamlTag: String? = "StringTypeTag"
}

fileprivate struct SimpleWithTagAndAnchor: SimpleProtocol, YamlTagProviding, YamlAnchorProviding {
    let nested: NestedStruct
    let intValue: Int
